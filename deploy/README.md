...
阿里云首尔服务器已运行 `2026.09.24-1`：API、MCP 和知识索引 Worker 使用此版镜像；模拟企业系统沿用 `2026.07.28-1`。实际部署根目录是 `/opt/bizorch`，公开入口为 `https://bizorch.nexmart.tech`。发布前备份保存在 `/opt/bizorch/backups/pre-2026.09.24-1`，包含两套 MySQL 数据库、旧配置与停止写入后归档的 Chroma、checkpoint 和上传文件。

2026-09-25 的前端性能优化版为 `2026.09.25-frontend-2`，位于 `/www/wwwroot/bizorch.nexmart.tech/releases/2026.09.25-frontend-2`，`current` 软链接指向该版本；上一版 `2026.09.25-frontend-1` 仍保留，可单独切回。该次只更新 Nginx 托管的静态文件，后端容器与数据未变。上线后已验证首页、前端资源和 API 健康检查，并撤销临时 SSH 公钥。
